...
const categoryIcons: Record<string, { icon: React.ElementType, color: string }> = {
  'شحن الألعاب والتطبيقات': { icon: Gamepad2, color: 'text-cyan-400' },
  'شحن ألعاب': { icon: Gamepad2, color: 'text-cyan-400' },
  'البطاقات': { icon: Gift, color: 'text-teal-400' },
  'بطاقات هدايا': { icon: Gift, color: 'text-teal-400' },
  'تطبيقات': { icon: AppWindow, color: 'text-sky-400' },
  ...
  'بطاقات بنكية': { icon: CreditCard, color: 'text-[#1aa0a1]' },
  'خدمات أخرى': { icon: Star, color: 'text-teal-300' },
  'الكل': { icon: LayoutGrid, color: 'text-[#1aa0a1]' },
};
...
function ProductCardLogo({ url, name }: { url?: string, name: string }) {
  ...
  if (!url || hasError) {
    return (
      <div className="h-14 w-14 rounded-2xl bg-gradient-to-br from-[#1aa0a1]/30 via-teal-600/20 to-cyan-600/30 border border-white/20 p-2 flex items-center justify-center text-primary shadow-md group-hover:scale-105 transition-transform">
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isActive 
                      ? 'bg-gradient-to-r from-[#1aa0a1] via-teal-600 to-teal-700 text-white shadow-lg shadow-[#1aa0a1]/30 border-teal-400/30 scale-[1.02]' 
                      : 'glass-pill hover:bg-[#1aa0a1]/10 text-foreground border-white/10 hover:border-[#1aa0a1]/40'
                  )}
                >
